...
const removedText = 'Screened lanais can dry more slowly than open hardscape. We account for airflow, shade and moisture before applying breathable sealer.';

const grid = '<div class="article-grid"><div><h3>Patio Paver Sealing Wildlight</h3><p>Patio pavers are inspected for staining, low joints, previous sealer, drainage and shaded areas before cleaning and sealing.</p></div><div><h3>Paver Lanai Sealing Wildlight</h3><p>' + removedText + '</p></div></div>';
if (!html.includes(grid)) throw new Error('Original two-card Wildlight grid not found.');
...
const replacement = `<div class="article-grid pool-wildlight-wide-grid">
  <figure class="pool-wildlight-wide-photo">
    ...
    <figcaption class="pool-wildlight-wide-caption">
      <div class="pool-wildlight-wide-caption-main">
      ...
      <div class="pool-wildlight-wide-caption-side"><h3>Paver Lanai Sealing Wildlight</h3></div>
    </figcaption>
  </figure>
</div>`;
html = html.replace(grid, replacement);
...
.pool-wildlight-wide-photo{position:relative;width:100%;height:430px;margin:0;overflow:hidden;border:1px solid var(--pool-line);border-radius:22px;background:#eef9f7;box-shadow:0 14px 32px rgba(15,111,129,.13)}
.pool-wildlight-wide-photo img{display:block;width:100%;height:100%;object-fit:cover;object-position:center}
.pool-wildlight-wide-caption{position:absolute;inset:0;display:flex;align-items:flex-end;justify-content:space-between;gap:18px;padding:26px;background:linear-gradient(180deg,rgba(6,36,48,0) 42%,rgba(6,36,48,.68) 100%)}
.pool-wildlight-wide-caption-main,.pool-wildlight-wide-caption-side{background:rgba(11,54,88,.78);border:1px solid rgba(255,255,255,.28);border-radius:16px;padding:15px 17px;backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px)}
.pool-wildlight-wide-caption-main{max-width:610px}.pool-wildlight-wide-caption-side{max-width:290px}
.pool-wildlight-focus .pool-wildlight-wide-caption h3{margin:0 0 6px!important;color:#fff!important}.pool-wildlight-focus .pool-wildlight-wide-caption-side h3{margin-bottom:0!important}
.pool-wildlight-wide-caption p{margin:0!important;color:rgba(255,255,255,.92)!important;line-height:1.55!important}
@media(max-width:760px){.pool-wildlight-wide-photo{height:360px}.pool-wildlight-wide-caption{padding:16px;gap:10px;flex-direction:column;align-items:stretch;justify-content:flex-end}.pool-wildlight-wide-caption-main,.pool-wildlight-wide-caption-side{max-width:none;padding:12px 14px}}
</style>`;
...
if (html.includes(removedText)) throw new Error('Lanai paragraph was not removed.');
...
console.log('Expanded yulee-pool-sealing.png across both Wildlight cards and removed the lanai paragraph.');
