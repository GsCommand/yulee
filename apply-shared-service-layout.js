...
<header class="ys-header">
  <nav class="ys-nav ys-shell" aria-label="Primary navigation">
    ...
    <div class="ys-menu" id="ysMenu">
      <div class="ys-group ys-group--paver">
        <button class="ys-parent" type="button" aria-expanded="false">Paver Sealing</button>
        <div class="ys-mega">
      ...
      <div class="ys-group">
        <button class="ys-parent" type="button" aria-expanded="false">Service Areas</button>
        <div class="ys-mega">
      ...
      <div class="ys-group">
        <button class="ys-parent" type="button" aria-expanded="false">Pressure Washing</button>
        <div class="ys-mega">
      ...
      <div class="ys-group">
        <button class="ys-parent" type="button" aria-expanded="false">About</button>
        <div class="ys-mega">
...
<script>
(function(){
  var toggle=document.querySelector('.ys-toggle');
  var menu=document.getElementById('ysMenu');
  if(!toggle||!menu)return;
  toggle.addEventListener('click',function(){
    var open=menu.classList.toggle('is-open');
    toggle.setAttribute('aria-expanded',String(open));
  });
  document.querySelectorAll('.ys-group').forEach(function(group){
    var button=group.querySelector('.ys-parent');
    if(!button)return;
    button.addEventListener('click',function(e){
      if(window.innerWidth>900)return;
      e.preventDefault();
      var willOpen=!group.classList.contains('is-open');
      document.querySelectorAll('.ys-group').forEach(function(other){
        other.classList.remove('is-open');
        var b=other.querySelector('.ys-parent');
...
function pressurePills(items) {
  if (items.length !== 4) {
    throw new Error(`Pressure-washing hero must have exactly four existing trust-strip items; found ${items.length}.`);
  }
  return `<div class="ys-feature-wrap" aria-label="Exterior cleaning service highlights">
    <div class="ys-feature-strip">
      ${items.map(item => `<article class="ys-feature ys-feature--compact"><h3>${item}</h3></article>`).join('\n      ')}
    </div>
  </div>`;
}
...
for (const filename of targets) {
  ...
  // Freeze ranking-sensitive elements before any cosmetic rewrite.
  ...
  if (!titleBefore || !descriptionBefore || !canonicalBefore || h1BeforeAll.length !== 1) {
    throw new Error(`Protected SEO foundation could not be frozen for ${filename}.`);
  }
  const h1Exact = h1BeforeAll[0];

  const oldHeader = html.match(/<header\s+class="site-header">[\s\S]*?<\/header>/i);
  const oldHero = html.match(/<section\s+class="hero">[\s\S]*?<\/section>/i);
  ...
  if (!imageTag || !attr(imageTag, 'src')) throw new Error(`Hero image could not be identified in ${filename}.`);
  ...
  const imageAlt = attr(imageTag, 'alt');
  ...
  const newHero = `<section class="ys-hero">
    <img class="ys-hero-bg" src="${imageSrc}" alt="${imageAlt}" fetchpriority="high" />
    <div class="ys-hero-grid ys-shell">
      <div class="ys-glass">
        ${eyebrow}
        ${h1Exact}
        ${heroText}
  ...
  if (!html.includes('/shared-top-layout.css')) {
    ...
    html = html.replace(cssAnchor, `${cssAnchor}\n<link rel="stylesheet" href="/shared-top-layout.css?v=1" />`);
  }
  ...
  // Verify protected SEO elements are byte-for-byte unchanged and still singular where expected.
  ...
  if (h1AfterAll.length !== 1 || h1AfterAll[0] !== h1Exact || exactCount(html, h1Exact) !== 1) {
    throw new Error(`H1 changed or duplicated during cosmetic transform: ${filename}`);
  }
  if (!html.includes(imageSrc)) throw new Error(`Original hero image was lost during cosmetic transform: ${filename}`);

  fs.writeFileSync(file, html);
  console.log(`Applied shared top layout to ${filename} using ${imageSrc}`);
}
